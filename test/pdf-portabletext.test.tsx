...
const components: Partial<PortableTextReactComponents> = {
	...
	types: {
		...
		localCurrency: (props: PortableTextComponentProps<LocalCurrency>) => <Text> {props.value.sourceAmount} {props.value.sourceCurrency} </Text>,
		...
		code: (props: PortableTextComponentProps<Code>) => {
			return <Text style={defaultStylesFactory(baseFontSizePt)?.marks?.code}> ({props.value.language}){"\n"}{"\n"}{props.value.code} </Text>
		},
		quote: (props: PortableTextComponentProps<Quote>) => {
			return <Text style={{backgroundColor: props.value.background, color: props.value.color}}>{
				props?.value?.children?.map((child: any) => {
